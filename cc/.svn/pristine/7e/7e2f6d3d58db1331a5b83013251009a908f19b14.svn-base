...
public class PinyinUtilsPro {
	private static Logger logger = Logger.getLogger(PinyinUtilsPro.class);
	...
	private  String regx = "(,| |\\[|\\])";// 正则表达式，匹配字符串用
						...
						if (len == 1) { // 不是多音字
							...
							if(duoyinziPinyins.size()==1){
								...
								if (!flag) { 
									...
									logger.debug("default = " + duoyinziPinyins.get(0));
	...
	// 截取词组，并匹配拼音表中的词组
	private  boolean matchPinyins(String py,String chinese,int m,int n){
	...
	// 数组转换成集合，并且去掉空格
	private  List<String> arr2List(String[] strs) {
